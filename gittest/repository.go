...
const (
	...
	// DefaultRemoteBranchAlias is a remote tracking branch that points at
...
func InitRepository(t *testing.T, opts ...RepositoryOption) {
	...
	_, err = Exec(t, fmt.Sprintf("git init --bare --initial-branch %s %s", DefaultBranch, BareRepositoryName))
	require.NoError(t, err)
	...
	for _, commit := range options.Commits {
		_, err := Exec(t, fmt.Sprintf(`git commit --allow-empty -m "%s"`, commit))
		require.NoError(t, err)
	}
...
// of 0750 (drwxr-xr-x), and the file created with permissions of 0600 (-rw-------).
...
func appendToReadme(t *testing.T, entry LogEntry) {
	...
	defer func() {
		require.NoError(t, file.Close(), "failed to close README.md")
	}()
...
func ConfigSet(t *testing.T, pairs ...string) {
	...
	require.Equal(t, 0, len(pairs)%2, "mismatch in number of config pairs")
